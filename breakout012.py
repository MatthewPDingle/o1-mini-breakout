...
class Brick(pygame.sprite.Sprite):
    ...
    def explode(self):
        ...
        for brick in bricks:
            if brick != self and abs(brick.rect.x - self.rect.x) < explosion_range and abs(brick.rect.y - self.rect.y) < explosion_range:
                brick.kill()
...
# **Removed Initial Brick Creation**
...
def reset_game():
    ...
    # Recreate Bricks **Only when the level starts**
    # bricks = create_bricks(5, 10, current_level)
    # all_sprites.add(bricks)
    ...
    # Recreate Ball **Only when the level starts**
    # initial_ball = Ball(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2)
    # balls.add(initial_ball)
    # all_sprites.add(initial_ball)
...
def main():
    ...
    while running:
        ...
        if level_start:
            ...
            # Separate handling of pause to maintain functionality
            for event in pygame.event.get():
