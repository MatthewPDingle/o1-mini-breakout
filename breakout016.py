...
POWERUP_TYPES = ['expand_paddle', 'extra_life', 'multi_ball',
                'shrink_paddle', 'slow_ball', 'laser_paddle']
...
class Brick(pygame.sprite.Sprite):
    def __init__(self, x, y, hits=1, color=GREEN, explosive=False):
        super().__init__()
        self.width = 60
        self.height = 20
        self.hits = hits
    ...
    def hit(self):
        ...
        if self.hits > 0:
            ...
            # Trigger explosion with 10% chance
            if self.explosive or (random.random() < 0.1):
                self.explode()

    def explode(self):
        logging.info(f"Brick at ({self.rect.x}, {self.rect.y}) is exploding adjacent bricks.")
        Explosion(self.rect.centerx, self.rect.centery)
        adjacent_bricks = pygame.sprite.spritecollide(
            self, bricks, False,
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if level >= 3 and random.random() < 0.2:
                ...
                explosive = False
            brick = Brick(x, y, hits, color, explosive)
            bricks.add(brick)
