...
class Paddle(pygame.sprite.Sprite):
    ...
    def update(self, keys):
        # Removed: logging.debug("Updating Paddle position.")
        self.moving_left = False
        self.moving_right = False
        if keys[pygame.K_LEFT]:
            self.rect.x -= self.speed
            self.moving_left = True
            # Removed: logging.debug("Paddle moving left.")
        if keys[pygame.K_RIGHT]:
            self.rect.x += self.speed
            self.moving_right = True
            # Removed: logging.debug("Paddle moving right.")

        # Keep paddle within screen
        if self.rect.left < 0:
            self.rect.left = 0
            # Removed: logging.debug("Paddle hit the left boundary.")
        if self.rect.right > SCREEN_WIDTH:
            self.rect.right = SCREEN_WIDTH
            # Removed: logging.debug("Paddle hit the right boundary.")

        # Handle paddle expansion timer
        if self.expanded:
            self.expand_timer -= 1
            # Removed: logging.debug(f"Paddle expansion timer: {self.expand_timer}")
...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self):
        # Removed: logging.debug("Updating Ball position.")
        ...
        # Removed: logging.debug(f"Ball angle before movement: {angle_before} degrees.")
...
class Brick(pygame.sprite.Sprite):
    def __init__(self, x, y, hits=1, color=GREEN):
        super().__init__()
        # Removed: logging.debug(f"Creating Brick at ({x}, {y}) with {hits} hits.")
        self.width = 60
        self.height = 20
        self.hits = hits
        self.max_hits = hits
...
class PowerUp(pygame.sprite.Sprite):
    ...
    def update(self):
        # Removed: logging.debug("PowerUp '...' moved to...")
...
logging.debug("Initial bricks added to all_sprites.")

# Create Paddle (Handled Separately)
paddle = Paddle()
# Removed: all_sprites.add(paddle)
...
def handle_collisions():
    ...
    # Removed: logging.debug("Handling collisions.")
    ...
    for ball in balls:
        ...
        for brick in hit_bricks:
            ...
            if ball.prev_rect.bottom <= brick.rect.top and ball.speed_y > 0:
                ...
                # Fallback
                ball.speed_y *= -1
                logging.warning("Ball collision side undetermined. Y speed inverted as fallback.")
    ...
    for power in power_hits:
        if power.power_type not in ['expanded_paddle', 'extra_life']:
...
def apply_powerup(power_type):
    ...
    if power_type == 'expand_paddle':
        paddle.expand()
        # Removed: logging.info("Paddle expanded via power-up.")
    elif power_type == 'extra_life':
        lives += 1
        # Removed: logging.info(f"Extra life granted. Lives: {lives}.")
...
def draw_text(text, font, color, surface, x, y):
    ...
    # Removed: logging.debug(f"Drawing text: '{text}' at ({x}, {y}).")
